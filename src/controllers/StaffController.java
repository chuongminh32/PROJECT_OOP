...
public class StaffController {
    ...
    // lay info 1 nhan vien 
    public static Staff getAStaff(String staffId) {
        try (Connection conn = DBConnection.getConnection()) {
            String sql = "SELECT id, name, email, phone, position, hireDate, password FROM Staff WHERE id = ?";
            PreparedStatement prsm = conn.prepareStatement(sql);
            prsm.setString(1, staffId);
            ...
            if (rs.next()) {
                ...
                Staff newStaff = new Staff(id, name, email, phone, pos, hireDate, pass);
                return newStaff;
            }
        } catch (Exception e) {
            e.printStackTrace();
    ...
    public void deleteStaff(String id) throws SQLException {
        ...
        try (Connection connection = DBConnection.getConnection(); PreparedStatement statement = connection.prepareStatement(query)) {
            ...
            if (rowsDeleted > 0) {
                ...
                System.out.println("Can't find record in table!");
            }
        } catch (SQLException | ClassNotFoundException e) {
            e.printStackTrace();
        }
    ...
    public void insertStaff(Staff staff) throws SQLException, ClassNotFoundException {
        ...
        try (Connection connection = DBConnection.getConnection(); PreparedStatement statement = connection.prepareStatement(query)) {
            ...
            statement.setDate(6, staff.getHireDate());
            ...
            if (rowsInserted > 0) {
                ...
                System.out.println("Can't insert record in table!");
            }
        } catch (SQLException | ClassNotFoundException e) {
            throw e;
        }
    ...
    public List<Staff> findStaffByPartialFields(String id, String name, String email, String phoneNumber, String position, String hireDate, String password) throws SQLException, ClassNotFoundException {
        ...
        try (Connection connection = DBConnection.getConnection(); PreparedStatement pstm = connection.prepareStatement(sql.toString())) {
